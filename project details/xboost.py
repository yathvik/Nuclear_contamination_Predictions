...
# Cross-validation with proper MSE (convert once to positive)
cv_scores = cross_val_score(best_model, X_train, y_train, cv=5, scoring='neg_mean_squared_error')
cv_mse_scores = -cv_scores  # positive MSE

print("\nCross-validation MSE per fold:", cv_mse_scores)
print(f"Mean CV MSE: {cv_mse_scores.mean():.4f} ± {cv_mse_scores.std():.4f}")

# Generate accuracy plot (MSE across folds)
