...
__all__ = ['ImprovedGCN', 'build_topk_edges']
...
def run_training():
    ...
    if not all(c in X_train.columns for c in required_coords):
        raise ValueError(f"Missing coordinate columns: {required_coords}")
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            train_r2 = max(0, r2_score(y_train_orig, train_pred_orig))
            test_r2 = max(0, r2_score(y_test_orig, test_pred_orig))
    ...
    feat_imp_df.to_csv(f'gcn_feature_importance_{timestamp}.csv', index=False)
    ...
    rf_r2 = max(0, r2_score(y_test.ravel(), rf_pred))
    print("RandomForest MSE:", rf_mse, "R2:", rf_r2)

    rf_df = pd.DataFrame({'Actual': y_test.ravel(), 'RF_Predicted': rf_pred})
    rf_df.to_csv(f'rf_predictions_{timestamp}.csv', index=False)
